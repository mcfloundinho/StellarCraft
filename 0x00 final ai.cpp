...
//		Move(GetStatus()->objects[0].id, speed2);
	...
	GO;
	for (;;) {
		if ((!me.skill_cd[SHIELD]) && (me.short_attack_casting == -1) && (me.long_attack_casting == -1)&&me.skill_level[SHIELD]>2) {
			WAIT;
			Shield(me.id);
			GO;
		}
...
void Strategy()
{
	if (ever_update) {
		...
		ever_update = 1;
//		std::cout << me.ability << std::endl;
		update(norm_update, &norm_counter);
	}
	if (opponent_number)
	{
		if ((!ever_update)&&dist(opponent[0].pos,me.pos)<3000) {
			if (!see_boss || dot_product(Minus(boss.pos, me.pos), Minus(opponent[0].pos, me.pos)) < 0) {
				for (;;) {
					me = GetStatus()->objects[0];
					speed = Minus(opponent[0].pos, me.pos);
					speed = MaximumSpeed(speed);
					Move(me.id, speed);
					update(attack_update, &attack_counter);
					//check dashable
					dash(me);
					if (dist(me.pos, opponent[0].pos) - me.radius - opponent[0].radius < kShortAttackRange[me.skill_level[SHORT_ATTACK]]-200)
						short_attack(me);
					if (dist(me.pos, opponent[0].pos) - me.radius - opponent[0].radius < kLongAttackRange[me.skill_level[LONG_ATTACK]-200])
						long_attack(me, opponent[0]);
					if (me.health < 0.5*me.max_health) break;
					if (me.radius < (kEatableRatio*opponent[0].radius+100)) break;
					initial_for_opponent();
				}
			}
		}
		dash(me);
...
int update(int* list,int* counter) {
//	std::cout << counter << " " << list[counter] << std::endl;
